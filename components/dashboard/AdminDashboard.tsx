...
const ProductivityChart: React.FC<{ stats: any }> = ({ stats }) => {
    ...
    const formatCurrency = (amount: number, decimals = 4) => {
...
const AdminDashboard: React.FC = () => {
    ...
    const handleUpdateLink = async () => {
        try {
            ...
            const message = err instanceof ApiError ? err.userMessage : "Failed to update link.";
            alert(message);
        }
    };
        ...
        try {
            ...
            for (const tableName in allData) {
                if (allData[tableName].length > 0) {
                    downloadCSV(allData[tableName], `export_${tableName}`);
                }
            }
        } catch (err) {
            const message = err instanceof ApiError ? err.userMessage : "An error occurred while downloading data.";
            alert(message);
        } finally {
    ...
    const handleDownloadExpiring = async () => {
        try {
            ...
            const message = err instanceof ApiError ? err.userMessage : "Could not download the list of expiring users.";
            alert(message);
        }
    };
    ...
    return (
        <div>
            ...
            <main className="p-4 sm:p-8 bg-gray-50 dark:bg-gray-900/50 min-h-[calc(100vh-112px)]">
                {view === 'dashboard' ? renderDashboard() : <IntegrationSettings onBack={() => setView('dashboard')} />}
            </main>
        </div>
    );
};
